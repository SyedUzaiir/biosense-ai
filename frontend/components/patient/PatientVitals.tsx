...
const METRIC_CONFIG: Record<string, { label: string; max?: number; unit?: string; highBad?: boolean; type?: 'text' }> = {
    sbp_mean_24h: { label: 'Systolic BP', max: 200, unit: 'mmHg', highBad: true },
    dbp_mean_24h: { label: 'Diastolic BP', max: 120, unit: 'mmHg', highBad: true },
    diabetesStatus: { label: 'Diabetes Status', type: 'text' },
    glucose_mean_24h: { label: 'Mean Glucose', max: 250, unit: 'mg/dL', highBad: true },
    hba1c: { label: 'HbA1c', max: 14, unit: '%', highBad: true },
    bmi: { label: 'BMI', max: 40, unit: 'kg/m²', highBad: true },
    insulin_any_24h: { label: 'Insulin (24h)', type: 'text' },
    hr_mean_24h: { label: 'Heart Rate', max: 150, unit: 'bpm', highBad: true },
    spo2_mean_24h: { label: 'SpO2', max: 100, unit: '%', highBad: false },
    temp_mean_24h: { label: 'Temperature', max: 42, unit: '°C', highBad: true },
};
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mt-6 border-t border-slate-100 pt-6">
                <div className="p-3 bg-slate-50 rounded-lg flex flex-col items-center justify-center text-center">
                    ...
                    <span className={cn("text-sm font-bold", patient.hypo_events_24h > 0 ? "text-red-600" : "text-emerald-600")}>
                        {patient.hypo_events_24h > 0 ? `DETECTED (${patient.hypo_events_24h} events)` : 'NEGATIVE'}
                    </span>
                </div>
                <div className="p-3 bg-slate-50 rounded-lg flex flex-col items-center justify-center text-center">
                    <span className="text-xs text-slate-500 uppercase tracking-wider mb-1">Hyperglycemia</span>
                    <span className={cn("text-sm font-bold", patient.hyper_events_24h > 0 ? "text-red-600" : "text-emerald-600")}>
                        {patient.hyper_events_24h > 0 ? `DETECTED (${patient.hyper_events_24h} events)` : 'NEGATIVE'}
                    </span>
                </div>
            </div>
        </div>
