...
export default function CheckerPage() {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const score = Math.round(data.risk_probability * 100);
            const prediction = data.risk;
            ...
            const trajectoryData = data.trajectory.map((val: number, hour: number) => ({
                hour: `${hour}h`,
                glucose: val
            }));
            ...
            toast.success("AI prediction calculated successfully!");
        } catch (error) {
            console.error("API error:", error);
            toast.error("Prediction service offline. Utilizing fallback prediction.");
            
            // Fallback
            setTimeout(() => {
                const score = Math.floor(Math.random() * 60) + 20; // 20-80
                let prediction = "Stable 🟢";
                if (score > 70) prediction = "High Risk 🔴";
                else if (score > 40) prediction = "At Risk 🟡";

                const chartData = [
                    { name: 'Your BMI', value: form.bmi, standard: 22 },
                    { name: 'Your HbA1c', value: form.hba1c, standard: 5.0 },
                    { name: 'Your Glucose', value: form.glucose_random / 20, standard: 5.0 },
                ];

                const trajectoryData = Array.from({ length: 25 }, (_, hour) => ({
                    hour: `${hour}h`,
                    glucose: Math.round(form.glucose_random + 15 * Math.sin(hour / 3) + Math.random() * 5)
                }));

                setResult({ score, prediction, chartData, trajectoryData, topFeatures: ['BMI', 'Age', 'Glucose'] });
            }, 1000);
        } finally {
            setLoading(false);
        }
                    ...
                    <AnimatePresence>
                        {result && (
                            ...
                            >
                                ...
                                {result.topFeatures && (
                                    <div className="mb-6">
                                        <p className="text-xs font-semibold text-slate-500 mb-2">Top Risk Contributors:</p>
                                        <div className="flex flex-wrap gap-2">
                                            {result.topFeatures.map((feat: string) => (
                                                <span key={feat} className="text-xs font-semibold px-2.5 py-1 bg-red-50 text-red-700 rounded-lg border border-red-100">
                                                    {feat}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                <div className="h-44 mb-6">
                                    <p className="text-xs font-semibold text-slate-500 mb-2">Biomarker Comparison:</p>
                                    ...
                                    <p className="text-center text-[10px] text-slate-400 mt-1">Value Comparison (Red = Abnormal)</p>
                                </div>

                                {result.trajectoryData && (
                                    <div className="border-t border-slate-100 pt-4 mb-6">
                                        <p className="text-xs font-semibold text-slate-500 mb-2">Projected 24h Risk Trajectory:</p>
                                        <div className="h-44">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <LineChart data={result.trajectoryData}>
                                                    <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                                    <XAxis dataKey="hour" fontSize={8} interval={4} />
                                                    <YAxis fontSize={8} domain={[60, 'auto']} />
                                                    <Tooltip />
                                                    <Line type="monotone" dataKey="glucose" stroke="#ef4444" strokeWidth={2} dot={false} activeDot={{ r: 4 }} name="Glucose" />
                                                    <Line type="monotone" dataKey="reference" stroke="#cbd5e1" strokeDasharray="5 5" name="Normal limit (180)" />
                                                </LineChart>
                                            </ResponsiveContainer>
                                            <p className="text-center text-[10px] text-slate-400 mt-1">Simulated blood glucose levels (mg/dL)</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                <button
                                    ...
                                    className="w-full border border-slate-200 text-slate-600 rounded-lg py-2.5 text-sm font-medium hover:bg-slate-50 flex items-center justify-center gap-2 transition"
                                >
