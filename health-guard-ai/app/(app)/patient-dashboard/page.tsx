  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
        try {
            ...
            emotionResult = data.emotion; // Expecting string with emoji
            confidence = Math.round(data.confidence * 100);
        } catch (error) {
            // Fallback gracefully
            const emotions = ['😊 Stable Psychological State', '😐 Baseline Emotional State', '😫 Elevated Stress Response', '😰 High Stress Indicators', '😢 Mild Depressive Indicators'];
            emotionResult = emotions[Math.floor(Math.random() * emotions.length)];
            confidence = Math.floor(Math.random() * (99 - 70 + 1) + 70); // random 70-99
            console.warn("Utilizing fallback emotion generation:", error);
        }
    }
    ...
    } else if (diabetesPrediction.includes('Stable') && emotionResult.includes('Stable')) {
        insight = "Stable metabolic profile with no significant psychological stress.";
    }
